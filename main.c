...
static void set_output_file(char *filename) {
  ...
  while (*mon_dir) {
    ...
    if (!strcmp(dir, *mon_dir)) {
      ...
      fprintf(stderr, "Error: Writing output file in a monitored dir is "
	      "not a good idea.\n");
...
static int check_params(char **argv) {
  ...
  while ((argv[i] != NULL) && strcmp(argv[i], "--")) {
    ...
    if (strcmp("-p", argv[i]) == 0) {
      ...
      if (argv[i] == NULL) {
	fprintf(stderr, "Error: payload option requires a dynamic library"
		" to load.\n");
	usage();
	exit(1);
      }
  ...
  if (file_nb == 0) {
    ...
    if (get_option(do_kill)) {
      ...
      g_params->files[n] = existing_parent_dir(g_params->target_file);
      file_nb++;
    }
  ...
  if (**cmd != '/' && (**cmd != '.' && *(*cmd + 1) != '/')) {
    ...
    if (ptr == NULL)
      fatal("Command can't be found in the PATH.  Try absolute path.\n");
